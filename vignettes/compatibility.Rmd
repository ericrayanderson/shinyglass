...
| Stack | Status | Notes |
|-------|--------|-------|
| `fluidPage` / `navbarPage` | Supported | Drop-in `theme = glass_theme()` or `glass_page()` |
...
- For plotly dark mode, pass transparent `paper_bgcolor` / `plot_bgcolor` in
  `layout()` (or `plotly_glass()`) — see `plotly-gt-demo.R`; CSS handles the
  modebar. Use `plot_surface = "opaque"` when the chart itself needs a panel.
- For print / chromote / PDF, call `glass_flatten()` or open with
  `?glass_flatten=1` so backdrop blur does not wash out the capture.
