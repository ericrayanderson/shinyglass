...
* Shiny User Showcase coverage (`inst/scripts/visual-test-shiny-gallery.R`)
  for rstudio/shiny-gallery apps (hangman, life-of-pi, one-source-indy,
  lake-profile-dashboard, nyc-metro-vis, …). Glass patching now finds
  nested `ui/ui.R` / `app.r`, allows nested `bootstrapPage` inside
  `fluidPage`, and fixes deprecated `plan(multiprocess)`.
