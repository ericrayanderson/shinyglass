...
* `glass_theme(plot_surface = "opaque")` densifies plot, plotly, gt, and DT
  ...
  `glass_plot_surface_input()`, `update_glass_theme(plot_surface = )`,
  `window.shinyglass.setPlotSurface()`, or `.glass-plot-surface-opaque`.
  `theme_glass()`, `plotly_glass()`, and `gt_theme_glass()` accept
  `surface = "opaque"`. `dt_options_glass()` is an optional DT helper.
* Named wallpaper scenes `aurora`, `harbor`, and `grove` join
  `tahoe` / `dusk` / `mesh`. `glass_scenes()` / `glass_scene_input()` list
  and switch them. User `wallpaper=` photos are blurred and washed
...
* Flatten mode for print, PDF, chromote, and screenshots:
  `glass_flatten()`, `glass_theme(flatten = )`, `update_glass_theme()`,
  `window.shinyglass.setFlatten()` / `.enterFlatten()` / `.exitFlatten()`,
  and `?glass_flatten=1`. `@media print` does the same automatically.
* Public `--glass-*` tokens: `glass_css_tokens()`, `glass_token_pack()`,
  `glass_add_tokens()`, `glass_theme(tokens = )`, and
  `update_glass_theme(tokens = )` / `window.shinyglass.setTokens()`.
...
* Stronger scroll-edge toolbar (`body.glass-scroll-edge`) when content
  ...
  Tinted.
* Deeper bslib Liquid Glass for `page_navbar` / navsets, nested cards and
  value boxes, sidebars, accordion, tooltips/popovers, and modals.
  Focus-visible rings and overlay stacking (tooltip 1090, popover 1085,
  menus 1080) apply to every `glass_theme()` app.
...
* `glass_theme()` no longer errors when `scene` is the default name vector
  (R 4.4+ `&&` length check) or a subset of scene names.
